...
import type { TrackedOrder, TrackedShipment, TrackingStatus, TrackingEvent } from '../data/delivery';
...
const PIPELINE: { status: TrackingStatus; label: string }[] = [
...
const TERMINAL_STATUSES: TrackingStatus[] = ['delivered', 'cancelled', 'returned'];

const STATUS_ADVANCE_NOTES: Record<TrackingStatus, string> = {
  placed: 'Order placed by customer',
  confirmed: 'Order confirmed by merchant',
  processing: 'Packed and processing at facility',
...
function FlyToOrder({ order }: { order: { currentLat: number; currentLng: number } | null }) {
  const map = useMap();
  useEffect(() => {
    if (order) {
...
function normalizeOrder(order: TrackedOrder): DisplayItem {
  return {
    id: order.id,
    kind: order.kind ?? "order",
    label: order.productName,
    image: order.productImage,
    qtyText: `${order.quantity} ×`,
...
function normalizeShipment(shipment: TrackedShipment): DisplayItem {
  return {
    id: shipment.id,
    kind: "shipment",
    label: shipment.title,
    image: "🚢",
    qtyText: `${shipment.weight} ${shipment.weightUnit}`,
    ...
    customerName: shipment.consignee,
    customerPhone: shipment.contactPhone,
  };
}

export default function OrderTrackingPage({ onNavigate, scope = "all", embedded = false }: Props) {
  ...
  const items = useMemo<DisplayItem[]>(() => {
    ...
    if (scope !== "shipments") {
      orders.forEach((o) => list.push(normalizeOrder(o)));
    }
    if (scope !== "orders") {
      shipments.forEach((s) => list.push(normalizeShipment(s)));
    }
    return list;
  }, [orders, shipments, scope]);

  const [selectedOrder, setSelectedOrder] = useState<DisplayItem | null>(null);
  ...
  const stats = useMemo(() => ({
    ...
    inTransit: items.filter((o) => ['in_transit', 'out_for_delivery'].includes(o.status)).length,
  }), [items]);

  const handleSelectOrder = useCallback((order: DisplayItem) => {
  ...
  const refreshSelected = useCallback(() => {
    setSelectedOrder((prev) => {
      if (!prev) return prev;
      if (prev.kind === 'shipment') {
        const fresh = useStore.getState().shipments.find((s) => s.id === prev.id);
        return fresh ? normalizeShipment(fresh) : prev;
      }
      const fresh = useStore.getState().orders.find((o) => o.id === prev.id);
      return fresh ? normalizeOrder(fresh) : prev;
    });
  }, []);
  ...
  const advanceOrderStatus = useCallback(() => {
    ...
    const next = idx >= 0 && idx < PIPELINE.length - 1 ? PIPELINE[idx + 1].status : null;
    if (!next) return;
    const lat = order.currentLat + (order.destinationLat - order.currentLat) * 0.4;
    const lng = order.currentLng + (order.destinationLng - order.currentLng) * 0.4;
    const location = next === 'delivered' || next === 'out_for_delivery'
      ? order.destinationCity
      : `${order.originCity} → ${order.destinationCity}`;
    const note = STATUS_ADVANCE_NOTES[next];
    const event: TrackingEvent = { status: next, timestamp: new Date().toISOString(), location, lat, lng, note };
    if (order.kind === 'shipment') {
      updateShipment(order.id, { status: next, currentLat: lat, currentLng: lng, events: [...order.events, event] });
    } else {
      updateOrderStatus(order.id, next, note, lat, lng, location);
    }
    refreshSelected();
  }, [selectedOrder, updateOrderStatus, updateShipment, refreshSelected]);

  const cancelOrder = useCallback(() => {
    const order = selectedOrder;
    if (!order || TERMINAL_STATUSES.includes(order.status)) return;
    const event: TrackingEvent = { status: 'cancelled', timestamp: new Date().toISOString(), location: order.originCity, lat: order.currentLat, lng: order.currentLng, note: 'Order cancelled' };
    if (order.kind === 'shipment') {
      updateShipment(order.id, { status: 'cancelled', currentLat: order.currentLat, currentLng: order.currentLng, events: [...order.events, event] });
    } else {
      updateOrderStatus(order.id, 'cancelled', 'Order cancelled', order.currentLat, order.currentLng, order.originCity);
    }
    refreshSelected();
  }, [selectedOrder, updateOrderStatus, updateShipment, refreshSelected]);
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                <p className="text-[13px] font-semibold text-ink truncate">{order.label}</p>
                                <span className={`shrink-0 px-1.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wide ${order.kind === "shipment" ? "bg-[#AF52DE]/15 text-[#AF52DE]" : "bg-[#007AFF]/15 text-[#007AFF]"}`}>
                                  {order.kind === "shipment" ? "Shipment" : "Order"}
                                </span>
                              </div>
              ...
              {filteredOrders.map((order) => (
                ...
                >
                  <Popup className="custom-popup">
                    <div className="p-1">
                      <p className="font-semibold text-[13px]">{order.label}</p>
                ...
                <div className="sticky top-0 z-10 bg-surface/90 backdrop-blur-xl border-b border-glass-border p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-[14px] font-bold text-ink">{selectedOrder.label}</p>
                  ...
                  {!TERMINAL_STATUSES.includes(selectedOrder.status) && (() => {
                    const idx = PIPELINE.findIndex((p) => p.status === selectedOrder.status);
                      ...
                      <div className="mt-3">
                        <div className="flex items-center gap-1 mb-2">
                          {PIPELINE.map((p, i) => {
                            const reached = idx >= i;
                            return (
                              <div key={p.status} className="flex-1">
                                <div
                                  className="h-1 rounded-full transition-colors"
                                  style={{ background: reached ? STATUS_COLORS[p.status] : 'rgba(128,128,128,0.18)' }}
                                />
                                <p
                                  className="text-[8px] text-center mt-1 truncate font-semibold"
                                  style={{ color: reached ? STATUS_COLORS[p.status] : '#8E8E93' }}
                                >
                                  {p.label}
                                </p>
                              </div>
                        ...
                        <div className="flex gap-2">
                          {next && (
                            <button
                              ...
                              className="flex-1 h-9 rounded-[10px] bg-[#007AFF] text-white text-[12px] font-semibold flex items-center justify-center gap-1.5 hover:bg-[#007AFF]/90 active:scale-[0.98] transition-all"
                            >
                              Advance to {STATUS_LABELS[next.status]}
                              <ArrowRight className="h-3.5 w-3.5" />
                            </button>
                          )}
                          <button
                            onClick={cancelOrder}
                            className="h-9 px-3 rounded-[10px] bg-surface-secondary/60 text-[12px] font-semibold text-ink-tertiary hover:text-error hover:bg-error/10 transition-colors flex items-center gap-1"
                          >
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="p-3 rounded-[12px] bg-surface-secondary/40 border border-glass-border">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-[12px] text-ink-tertiary">{selectedOrder.kind === "shipment" ? "Weight" : "Quantity"}</span>
                      <span className="text-[13px] font-semibold text-ink">{selectedOrder.qtyText}</span>
                    </div>
                    <div className="flex justify-between items-center mb-2">
