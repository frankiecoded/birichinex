          ...
          <div className="divide-y divide-glass-border">
            ...
            {filtered.map((contact, i) => (
              <motion.div
                ...
                className="flex items-center gap-4 px-5 py-4 hover:bg-surface-secondary/40 transition-colors cursor-pointer group"
                onClick={() => setSelectedContact(contact)}
              >
                <div className="h-10 w-10 rounded-full bg-night flex items-center justify-center shrink-0">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <ArrowUpRight className="h-4 w-4 text-ink-quaternary group-hover:text-brand transition-colors" />
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <AnimatePresence>
        {selectedContact && (
          <motion.div
            ...
            className="fixed inset-0 z-50 flex items-center justify-center p-4"
          >
            <div className="absolute inset-0 bg-scrim backdrop-blur-sm" onClick={() => setSelectedContact(null)} />
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 8 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 8 }}
              transition={{ duration: 0.25, ease: [0.22, 1, 0.36, 1] }}
              className="relative w-full max-w-lg glass-material-lg specular-sheen rounded-[20px] shadow-2xl overflow-hidden max-h-[85vh] overflow-y-auto"
            >
              <div className="p-7">
                <div className="flex items-start justify-between mb-5">
                  <div className="flex items-center gap-4">
                    <div className="h-14 w-14 rounded-full bg-night flex items-center justify-center shrink-0">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h2 className="text-title font-bold text-ink tracking-tight">{selectedContact.name}</h2>
                        <Badge variant={STATUS_STYLES[selectedContact.status].variant} size="sm">
                          {STATUS_STYLES[selectedContact.status].label}
                        </Badge>
                      </div>
                      <p className="text-caption text-ink-tertiary mt-0.5">{selectedContact.company} · {selectedContact.role}</p>
                    </div>
                  </div>
                  <button onClick={() => setSelectedContact(null)} className="h-8 w-8 rounded-[8px] flex items-center justify-center text-ink-quaternary hover:text-ink hover:bg-surface-secondary/80 transition-colors">
                    <X className="h-4 w-4" strokeWidth={1.5} />
                  </button>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
                  <div className="bg-surface-secondary/50 border border-glass-border rounded-[12px] p-3">
                    <p className="text-[10px] uppercase tracking-wider font-bold text-ink-quaternary mb-1">Email</p>
                    <a href={`mailto:${selectedContact.email}`} className="text-caption font-semibold text-ink hover:text-brand transition-colors break-all">
                      {selectedContact.email}
                    </a>
                  </div>
                  <div className="bg-surface-secondary/50 border border-glass-border rounded-[12px] p-3">
                    <p className="text-[10px] uppercase tracking-wider font-bold text-ink-quaternary mb-1">Phone</p>
                    <a href={`tel:${selectedContact.phone}`} className="text-caption font-semibold text-ink hover:text-brand transition-colors">
                      {selectedContact.phone}
                    </a>
                  </div>
                  <div className="bg-surface-secondary/50 border border-glass-border rounded-[12px] p-3">
                    <p className="text-[10px] uppercase tracking-wider font-bold text-ink-quaternary mb-1">Added</p>
                    <p className="text-caption font-semibold text-ink">
                      {new Date(selectedContact.createdAt).toLocaleDateString()}
                    </p>
                  </div>
                  <div className="bg-surface-secondary/50 border border-glass-border rounded-[12px] p-3">
                    <p className="text-[10px] uppercase tracking-wider font-bold text-ink-quaternary mb-1">Last contact</p>
                    <p className="text-caption font-semibold text-ink">
                      {new Date(selectedContact.lastContactAt).toLocaleDateString()}
                    </p>
                  </div>
                </div>

                <div className="mb-5">
                  <p className="text-caption font-semibold text-ink-secondary mb-2">Status</p>
                  <div className="flex gap-2">
                    {FILTER_OPTIONS.filter((f) => f !== "all").map((s) => (
                      <button
                        key={s}
                        onClick={() => {
                          ...
                          setSelectedContact({ ...selectedContact, status: s, lastContactAt: new Date().toISOString() });
                        }}
                        className={`h-8 px-3 rounded-full text-caption font-semibold transition-all duration-200 cursor-pointer ${
                          selectedContact.status === s
                            ? "bg-emphasis text-on-emphasis"
                            : "bg-surface-secondary/60 border border-glass-border text-ink-tertiary hover:text-ink"
                        }`}
                      >
                ...
                {selectedContact.tags.length > 0 && (
                  <div className="mb-5">
                    <p className="text-caption font-semibold text-ink-secondary mb-2">Tags</p>
                    <div className="flex flex-wrap gap-1.5">
                      {selectedContact.tags.map((tag) => (
                        <span key={tag} className="px-2.5 py-1 rounded-full bg-brand/10 border border-brand/15 text-[11px] font-semibold text-brand-dark capitalize">
                          {tag.replace(/-/g, " ")}
                        </span>
                      ))}
                    </div>
                ...
                <div className="mb-6">
                  <p className="text-caption font-semibold text-ink-secondary mb-2">Notes</p>
                  <p className="text-caption text-ink leading-relaxed bg-surface-secondary/50 border border-glass-border rounded-[12px] p-3">
                ...
                <div className="flex gap-3">
                  ...
                  <Button
                    ...
                    onClick={() => { deleteContact(selectedContact.id); setSelectedContact(null); }}
                  >
                    Delete
                  </Button>
                </div>
