...
import {
  ...
  Key, ExternalLink, Shield,
} from "lucide-react";
import Badge from "../components/ui/Badge";
import Button from "../components/ui/Button";
import CursorSpotlight from "../components/three/CursorSpotlight";
import { useStore } from "../store/useStore";
import { chatWithAI, configureAI, isAPIConfigured, getAIConfig } from "../services/ai/api-client";
import type { AIContext, UserData } from "../services/ai/intent-engine";
...
function getGreeting(): string {
  const greetings = [
    "Hello! I'm your BirichiNex AI Business Advisor. I have comprehensive knowledge about the platform, sourcing strategies, pricing, finance, and growth planning. How can I help you today?",
    "Welcome! I can help you navigate BirichiNex's 19 business capabilities, optimize your operations, and develop winning strategies. What would you like to explore?",
    "Hi there! Whether you need help with dropshipping strategy, inventory management, financial planning, or market entry — I'm here to guide you. What's on your mind?",
  ];
...
function APISettingsModal({ onClose }: { onClose: () => void }) {
  const [provider, setProvider] = useState<"openai" | "anthropic" | "local">(getAIConfig().provider);
  ...
  const handleSave = () => {
    configureAI({
      ...
      model: provider === "openai" ? (model || "gpt-4o") : provider === "anthropic" ? (model || "claude-sonnet-4-20250514") : undefined,
    });
    setSaved(true);
            ...
            <div className="flex gap-2">
              {(["local", "openai", "anthropic"] as const).map((p) => (
                <button
                  ...
                  className={`flex-1 h-10 rounded-[10px] text-[13px] font-medium border transition-all ${
                    provider === p
...
export default function AIAdvisorPage() {
  const {
    ...
    loyalty, cart, currentTier, selectedCurrency, user,
  } = useStore();
  ...
  const userData: UserData | undefined = useMemo(
    ...
    [dataToggle, user, currentTier, loyalty, inventoryItems, transactions, contacts, cart, selectedCurrency],
  );
  ...
  const handleSend = useCallback(
    async (text?: string) => {
      ...
      try {
        ...
        const errorMsg: AdvisorMessage = {
          id: crypto.randomUUID(),
          role: "assistant",
          content: "I apologize, but I encountered an error processing your request. Please try again or configure an AI provider in settings.",
          timestamp: new Date().toISOString(),
        };
        setConversations((prev) =>
          prev.map((c) =>
    ...
    [input, loading, activeConvId, dataToggle, userData, conversations],
  );
  ...
  return (
    <div className="flex h-[calc(100dvh-3.5rem)] overflow-hidden">
      ...
      <AnimatePresence>
        {showAPISettings && (
          <APISettingsModal onClose={() => { setShowAPISettings(false); setApiReady(isAPIConfigured()); }} />
        )}
      </AnimatePresence>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[12px] text-ink-tertiary">
                  {apiReady ? "Connected to live AI provider" : "Pattern matching • Knowledge base"} • 62 knowledge entries
                </p>
              </div>
            </div>
              ...
              >
                ...
                {!apiReady && (
                  ...
                  >
                    ...
                    Connect an AI provider for enhanced responses
        ...
        <div className="shrink-0 border-t border-glass-border bg-surface/30 backdrop-blur-xl">
          <div className="max-w-3xl mx-auto px-6 py-4">
            ...
            <p className="text-[10px] text-ink-quaternary text-center mt-2.5">
              {apiReady
                ? "Connected to live AI provider. Responses powered by your configured API."
