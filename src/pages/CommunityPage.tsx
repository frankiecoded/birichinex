...
export default function CommunityPage({ onNavigate }: CommunityPageProps) {
  ...
  // ── Directory State ──────────────────────────────────────────────────────
  const [businesses] = useState<BusinessProfile[]>(SEED_BUSINESSES);
                  ...
                  <TiltCard>
                    <GlassCard padding="lg" hover className="h-full flex flex-col" onClick={() => setSelectedBusiness(biz)}>
                      <div className="flex items-center gap-3 mb-3">
                        <div className="h-12 w-12 rounded-full bg-ink flex items-center justify-center shrink-0">
            ...
            {selectedBusiness && (
              <div className="space-y-5">
                <div className="flex items-center gap-4">
                  <div className="h-16 w-16 rounded-full bg-ink flex items-center justify-center shrink-0">
                ...
                <div className="flex gap-3">
                  {selectedBusiness.connected ? (
                    <Button variant="secondary" size="lg" icon={<Check className="h-4 w-4" />} fullWidth>
                      Connected
                    </Button>
                  ) : (
                    <Button variant="primary" size="lg" icon={<Users className="h-4 w-4" />} fullWidth>
                      Request Connection
                    </Button>
                  )}
                </div>
        ...
        >
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {SEED_BUSINESSES.filter((b) => b.connected).map((biz, i) => (
                <motion.div
                  key={biz.id}
                  initial={{ opacity: 0, y: 12 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  <GlassCard padding="md" hover className="text-center">
                    <div className="h-12 w-12 rounded-full bg-ink mx-auto mb-2 flex items-center justify-center">
                      <span className="text-white font-bold text-lg">{biz.name.charAt(0)}</span>
                    </div>
              ...
              {SEED_BUSINESSES.filter((b) => b.connected).length === 0 && (
