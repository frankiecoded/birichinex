...
export default function ShoppingShell({
  ...
  const SHOP_FOOTER_NAV: Record<string, string> = {
    Fashion: "category:fashion",
    Technology: "category:technology",
    "New Arrivals": "category:fashion",
    "Best Sellers": "category:fashion",
    Deals: "deals",
  };
      ...
      <AnimatePresence>
        {!dismissedBanner && (
          <motion.div
            ...
            className="bg-night text-white text-center py-2 px-4 relative overflow-hidden"
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-10 mb-12">
              <div>
                ...
                <div className="space-y-2.5">
                  {Object.keys(SHOP_FOOTER_NAV).map((item) => (
                    <button
                      key={item}
                      onClick={() => handleFooterAction(SHOP_FOOTER_NAV[item])}
              ...
              <div>
                ...
                <div className="space-y-2.5">
                  {Object.keys(SHOP_FOOTER_SUPPORT).map((item) => (
                    <button
                      key={item}
                      onClick={() => handleFooterAction(SHOP_FOOTER_SUPPORT[item])}
                      className="block text-caption text-zinc-400 hover:text-white transition-colors duration-200"
                    >
                      {item}
                    </button>
